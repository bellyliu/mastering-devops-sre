...
def get_book(db: Session, book_id: int, redis_client=None):
    ...
    cache_key = f"mysql:book:{book_id}"
    ...
    if redis_client:
        ...
        if cached_book:
            ...
            # Return a dict that can be used by Pydantic
            return book_data
    
    # Get from database with relationship loaded
...
def get_books(
    ...
    """Get all books with optional filtering and price range"""
    ...
    if search:
        query = query.filter(
            or_(
                models.Book.title.like(f"%{search}%"),
                models.Book.author.like(f"%{search}%")
            )
        )
...
def create_book(db: Session, book: schemas.BookCreate, owner_id: int, redis_client=None):
    ...
    # Invalidate cache
    if redis_client:
        redis_client.delete("mysql:books:*")
...
def update_book(
    ...
    # Invalidate cache
    if redis_client:
        redis_client.delete(f"mysql:book:{book_id}")
        redis_client.delete("mysql:books:*")
